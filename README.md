...
For attached version-3 sessions, JSON adds `telemetry.harness` with aggregate assistant outcomes (labelled turns), `activity` counts for tool calls, tool results, compactions, and branch summaries, reconciled parent components and reported cost, bounded Pi token attribution, and independent bounded model-cost attribution. Complete scans report exact zeros and values; partial scans retain known lower bounds; unavailable authoritative values are `null`. Named cost buckets are assistant-only provider/model keys, while missing assistant keys, overflow keys, and optional tool/summary costs use separate buckets. Per-turn history publishes numeric reported cost and tool-call counts, but never cost source details or private content. Status is `complete`, `partial`, or `unavailable`; unavailable authoritative totals are `null`, not zero. `harness` is `null` for process-only rows and sessions without supported version-3 rich telemetry. In selected-session detail, surplus space shows a full input/output/cache-read/cache-write, cost, tool-call, model, and event history at width 90 or more, or compact total-token, cost, tool-call, model, and event rows below that width; the existing narrow Sessions `+` zoom provides that space on compact terminals. `--once` output remains unchanged. These model names are Pi metadata, not separate monitored agents.
