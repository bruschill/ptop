...
### Optional live-harness extension

On macOS and Linux, these explicit local commands manage the packaged Pi extension:
...
They do not change Pi settings or contact a network service. The command resolves Pi's agent directory from an absolute `PI_CODING_AGENT_DIR` (only exact `~` or `~/relative` expansion is accepted) or `~/.pi/agent`, then manages `extensions/ptop-live-harness.ts`. It refuses unsafe paths, symlinks, unknown or modified extension bytes, and lock contention. `status` is read-only and creates no directories or lock files.

To upgrade, run `ptop extension remove`, then `ptop extension install`. Run `/reload` or restart Pi only after both commands succeed. There is no backup, rollback, force, update, or restore command. An interrupted install can leave a partial or unknown fixed install temp before publication, a recognized two-link target/temp pair after publication, or an installed target with durability uncertainty after directory sync. An interrupted remove can leave removal durability uncertain after unlink. ptop never cleans these states automatically. Stop Pi and inspect the exact extension directory and file identities before removing any residue manually. JSONL and process-only monitoring are unchanged if optional live telemetry is absent.

The extension uses a private local AF_UNIX socket only. It stays inactive for a session if the session identifier cannot fit the bounded 512-byte protocol frame after JSON escaping; JSONL and process-only monitoring continue. If Pi crashes, its socket can remain. Stop every Pi process and verify the private `/tmp/ptop-<uid>` (Linux) or `/private/tmp/ptop-<uid>` (macOS) directory before manually removing a stale socket; ptop never removes runtime sockets automatically.

It does not retain or send prompts, assistant output, tool arguments, tool results, titles, paths, or raw events. Installing it is opt-in. After cross-tick process, attachment, socket, and peer-credential validation, JSON snapshots may include `telemetry.live_harness` with only phase, sampled pending-message state, source health, fixed provenance, observation time, stale state, and a fixed ptop reason. Process-only, unsupported, absent, ambiguous, or rejected live data remains `null`. Selected-session detail shows one `Live` line for healthy or stale observations; unavailable data adds no row. Text `--once` output is unchanged.
...
Parent usage includes every persisted branch entry once. Missing assistant usage still counts a turn and marks component totals partial. Missing usage on optional tool-result, compaction, and branch-summary records makes no contribution, while present invalid usage marks totals partial. A partial total with no complete component observation is unavailable, not zero; clean sessions with only a header or absent optional usage remain known zero. Token history keeps the latest 64 complete assistant component totals. Version-3 `telemetry.harness.history` also publishes at most 64 latest persisted assistant component observations in file order, preserving unavailable components as `null`, safe Pi provider/model attribution when available, and at most 64 positioned compaction or branch-summary markers. It never publishes entry IDs, timestamps, text, costs, stop reasons, summary content, or compaction token thresholds. Context is inferred from the latest persisted session entry because Pi navigation is not available to the passive parser; `active_leaf_id` remains in JSON for compatibility and is always `null`. Reported cost is independent of component completeness and appears only when every expected cost is valid and the attached session scan is complete. `totalTokens` is used only for the context baseline; it does not repair component totals. Parent usage and cost remain separate from fleet-run accounting.

For attached version-3 sessions, JSON adds `telemetry.harness` with aggregate assistant outcomes, reconciled parent components and reported cost, and bounded Pi model attribution. Status is `complete`, `partial`, or `unavailable`; unavailable authoritative totals are `null`, not zero. `harness` is `null` for process-only rows and sessions without supported version-3 rich telemetry. When the optional extension is validated, JSON also adds `telemetry.live_harness`. Healthy observations may contain a fixed live phase and sampled pending-message boolean. A stale observation clears both values, and unavailable data is `null`; old live values are never reused. Selected-session detail renders known healthy values as `Live <phase> · pending <yes/no>`, unknown healthy values as unavailable, and stale data as `Live stale · values unavailable`. Unavailable live telemetry adds no row. Surplus space shows a full component history graph at width 90 or more, or a compact total graph below that width; the existing narrow Sessions `+` zoom provides that space on compact terminals. `--once` output remains unchanged. These model names are Pi metadata, not separate monitored agents.
...
ptop does not retain or publish prompt text, assistant text, tool arguments, tool results, or child transcripts. Pi session parsing keeps identity, safe metadata, structured numeric telemetry, and the approved bounded version-3 assistant component history and positioned summary markers only. Fleet collection reads supported lifecycle status metadata, not child output logs or transcript files.

TUI, text snapshots, and JSON snapshots reduce child and orphan commands to executable labels where output leaves the collector. ptop performs no network requests. When the interactive TUI runs inside Herdr on macOS or Linux, it reads the verified Space name and sends the `ptop` name, the display token `ptop=running`, the previous name on restoration, and exact Herdr resource IDs back to the same local Herdr socket. Package installation, `ptop --update`, and `ptop extension` lifecycle operations are separate user-requested operations. The optional extension uses only local AF_UNIX IPC; it makes no Internet, IP, HTTP, RPC, subprocess, or monitoring network request.
