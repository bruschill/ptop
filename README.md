...
For attached version-3 sessions, JSON adds `telemetry.harness` with aggregate assistant outcomes, reconciled parent components and reported cost, and bounded Pi model attribution. Status is `complete`, `partial`, or `unavailable`; unavailable authoritative totals are `null`, not zero. `harness` is `null` for process-only rows and sessions without supported version-3 rich telemetry. In selected-session detail, surplus space shows a full component history graph at width 90 or more, or a compact total graph below that width; the existing narrow Sessions `+` zoom provides that space on compact terminals. `--once` output remains unchanged. These model names are Pi metadata, not separate monitored agents.
