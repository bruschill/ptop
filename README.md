...
Parent usage includes every persisted branch entry once. Missing assistant usage still counts a turn and marks component totals partial. Missing usage on optional tool-result, compaction, and branch-summary records makes no contribution, while present invalid usage marks totals partial. A partial total with no complete component observation is unavailable, not zero; clean sessions with only a header or absent optional usage remain known zero. Token history keeps the latest 64 complete assistant component totals. Context is inferred from the latest persisted session entry because Pi navigation is not available to the passive parser; `active_leaf_id` remains in JSON for compatibility and is always `null`. Reported cost is independent of component completeness and appears only when every expected cost is valid and the attached session scan is complete. `totalTokens` is used only for the context baseline; it does not repair component totals. Parent usage and cost remain separate from fleet-run accounting.

For attached version-3 sessions, JSON adds `telemetry.harness` with aggregate assistant outcomes, reconciled parent components and reported cost, and bounded Pi model attribution. Status is `complete`, `partial`, or `unavailable`; unavailable authoritative totals are `null`, not zero. `harness` is `null` for process-only rows and sessions without supported version-3 rich telemetry. The selected-session detail shows compact outcomes, model summaries, and reconciliation status. `--once` output remains unchanged. These model names are Pi metadata, not separate monitored agents.
