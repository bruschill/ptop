...
pub struct ClaudeCollector {
    ...
    /// User-configured Claude config directories from ptop config.
...
impl ClaudeCollector {
    ...
    fn refresh_config_dirs(&mut self, process_info: &HashMap<u32, process::ProcInfo>) {
        ...
        // Include CLAUDE_CONFIG_DIR from ptop's own environment
    ...
    /// of ptop itself. Other users' non-interactive (`claude --print`)
    /// invocations are still surfaced — only ptop's own summary children
    /// are filtered out.
    ...
    fn load_session(
        ...
        // Skip sessions whose PID is a descendant of ptop itself —
...
struct DiscoveryContext {
    ...
    /// ptop's own PID, threaded through so `load_session` can self-filter
...
fn build_discovery_context(
    ...
    for (path, _) in session_paths {
        ...
        // descended from ptop itself. Stale `sessions/{PID}.json` files
        // (crashed sessions) and ptop's own `claude --print` summary
...
fn extract_prompt_text(message: &Value) -> String {
    ...
    // Skip prompts generated by ptop's own summary generation (claude --print)
...
/// 1. `CLAUDE_CODE_EFFORT_LEVEL` env var (ptop's own env — only visible when
///    set in the user's shell before launching both ptop and claude)
...
///
/// Precedence (highest wins), matching Claude Code's own resolution order:
/// 1. `CLAUDE_CODE_MODEL` env var (ptop's own env — only visible when
///    set in the user's shell before launching both ptop and claude)
/// 2. `{cwd}/.claude/settings.local.json`
/// 3. `{cwd}/.claude/settings.json`
/// 4. `~/.claude/settings.local.json`
...
/// ptop's own env are the portable signals there.
///
...
";
    ...
    fn test_find_claude_pids_excludes_self_spawned_print_sessions() {
        // Simulate ptop running as PID 99 with a `claude --print` summary
        ...
        let mut got = ClaudeCollector::find_claude_pids(&process_info, ptop_pid);
        got.sort_unstable();
...
"#;
    ...
    fn test_load_session_overrides_sid_despite_print_sibling() {
        // Regression guard: ptop spawns `claude --print` for summary
        ...
        let ctx = build_discovery_context(&session_paths, &process_info, ptop_pid);
    ...
    fn test_load_session_keeps_user_spawned_print() {
        ...
        // launched by the *user* (not ptop) must be surfaced. Before the
        ...
        process_info.insert(
            ...
            ProcInfo {
                pid: user_print_pid,
                ppid: 1, // launched by the user, not ptop
                rss_kb: 512,
                cpu_pct: 0.5,
                command: "claude --print user-script".to_string(),
            },
    ...
    fn test_transcript_cache_evicts_old_sid_after_clear() {
        // Two-poll regression: on the first tick ptop parses the old
