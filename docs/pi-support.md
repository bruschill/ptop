...
The parent JSONL parser retains identity, model/provider metadata, thinking level, numeric usage, context size, compaction state, and safe activity labels. For attached version-3 sessions, JSON and selected-session detail may publish aggregate assistant outcomes, reconciled parent components and reported cost, at most 64 sorted Pi token and cost-attribution buckets (including unavailable, overflow, and unattributed optional-entry costs), status-bearing activity counts, independent cost attribution, and the approved bounded history projection: at most 64 latest assistant component observations in file order (with `null` gaps), nullable per-turn reported cost and tool-call counts, safe per-point Pi attribution, and at most 64 positioned compaction or branch-summary markers. History publishes no IDs, timestamps, text, outcomes, cost source details, summary data, or compaction `tokensBefore` values. `telemetry.harness` is `null` for process-only rows, versions 1 and 2, unsupported sessions, failed attachments, and inconsistent projections. Its unavailable authoritative totals are `null`; `history` is an object only when its separate bounded projection validates, otherwise `null` without suppressing valid aggregates. It does not retain or publish:
...
- prompt or assistant content in history, entry IDs, parent IDs, timestamps, cost source details or outcomes, summary content, or compaction token thresholds
...
Reported cost is reconciled independently from components. Every assistant requires a cost observation; optional entries require one only when their usage key is present. Version-3 harness activity reports tool-call blocks, tool results, compactions, and branch summaries across every accepted persisted branch. Complete scans expose exact zero or value; incomplete scans retain known lower bounds, while unavailable values are `null`. Invalid assistant content makes only the tool-call activity partial. Cost attribution has at most 64 assistant provider/(responseModel ?? model) buckets plus unavailable-assistant, overflow, and optional tool/summary buckets. Its known lower-bound total reconciles with reported cost when complete. Per-point cost and tool-call values are public only in the bounded latest-64 assistant history; cost source details and all tool, result, and summary payloads remain private. This is distinct from `AgentSession.compaction_count`, which remains latest-branch context inference. A valid `cost.total` can be included even when that entry's components are invalid. Cost is published only when every required cost is finite, non-negative, and has a finite checked accumulated total. Missing or invalid expected cost, incomplete tailing, parser limits, semantic loss, or duplicate IDs leave reported cost unavailable. Parent-session cost and usage remain separate from fleet-run accounting.
